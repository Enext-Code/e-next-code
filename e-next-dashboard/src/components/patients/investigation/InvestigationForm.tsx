...
export default function InvestigationForm({ patientId, isFromSearch = false, setActiveTab }: InvestigationFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await patientService.addInvestigation(
      ...
      if (response.success) {
        if (isFromSearch) {
          // If coming from search, go back to history page
          router.push(`/patients/${patientId}/history`);
        } else {
          // If normal flow, go back to history page
          router.push(`/patients/${patientId}/history`);
        }
      } else {
        setError(response.message || 'Failed to save investigation');
      }
    } catch (err) {
