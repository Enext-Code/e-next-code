...
interface FluidFormProps {
  ...
  isViewMode?: boolean;
  time: string;
  onSubmit: (time: string, values: FluidData) => void;
}
...
const FluidForm: React.FC<FluidFormProps> = ({
  ...
  const handleDeleteDrainage = (index: number) => {
    setFormValues(prev => {
      const updatedDrainages = prev.drainages.filter((_, i) => i !== index);
      const newValues = {
        ...prev,
    ...
    return (
      <div className={styles.section}>
        <div className={styles.sectionHeader}>
          ...
          {!isViewMode && (
            <button type="button" onClick={() => {
              const nextIndex = formValues.drainages.length + 1;
              setFormValues(prev => ({
                ...prev,
                drainages: [...prev.drainages, { name: `Drain ${nextIndex}`, quantity: null }]
              }));
            }} className={styles.addButton}>
              + Add More Infusions
            </button>
        ...
        <div className={styles.drainageGrid}>
          {formValues.drainages.map((drainage, index) => (
            <div key={index} className={styles.drainageRow}>
              <div className={styles.drainageHeader}>
                <label>Drainage {String(index + 1).padStart(2, '0')}</label>
                {!isViewMode && (
                  <button 
                    type="button" 
                    onClick={() => handleDeleteDrainage(index)}
                    className={styles.deleteButton}
                  >
                    ×
                  </button>
              ...
              <div className={styles.drainageContent}>
                <span className={styles.drainageName}>Drain {index + 1}</span>
                <div className={styles.inputWithUnit}>
                  {isViewMode ? (
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const quantity = value === "" ? null : parseFloat(value);
                        setFormValues(prev => {
                          const updatedDrainages = prev.drainages.map((item, i) => 
                            i === index ? { ...item, quantity } : item
                          );
  ...
  React.useEffect(() => {
    if (formValues.drainages.length === 0) {
      setFormValues(prev => ({
        ...prev,
        drainages: [
          { name: "Drain 1", quantity: null },
          { name: "Drain 2", quantity: null },
          { name: "Drain 3", quantity: null },
          { name: "Drain 4", quantity: null }
        ]
      }));
    }
  }, []);
